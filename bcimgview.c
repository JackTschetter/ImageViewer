...
struct image_info {
    ...
    void (*cleanup)(struct image_info *);  /* destructor callback */
...
/* Forward reference to the image pixels cleanup function */
void free_pixels(struct image_info *info);
...
struct image_info *parse_bcraw(FILE *fh) {
    ...
    info_footer->cleanup = &free_pixels;
    ...
    for (channel = 0; channel <= 2; channel++) {
        for (y = 0; y < info->height; y++) {
            ...
            while (x < info->width) {
                ...
                assert(num_pixels >= 0);
                if (num_pixels == 0 && buf_read_pos == 1) {
                    /* In most cases, any sequence of byte values
                       should be able to be interpreted as at least
                       one pixel, even if erroneously. But at the end
                       of the file it can happen that the last byte of
                       the file is interpreted as the start of a
                       codeword that could only be finished by reading
                       beyond the end of the file. This indicates an
                       erroneous file. */
                    format_problem = "invalid pixel at end of file";
                    return 0;
                }
                assert(num_pixels > 0);
                assert(num_pixels <= max_pixels + EXPANSION);
...
/* Free the image data associated with an image_info struct. This is
   the most commonly-used cleanup function, since all image formats
   use the pixels field. */
void free_pixels(struct image_info *info) {
    free(info->pixels);
}

/* Do all the cleanup associated with an image_info that is no longer
   needed: call the destructor if present, and then free the
   structure. */
...
int num_images_viewed = 0;

/* Keep track of how many images a user has opened in a single
   session. */
void count_image(void) {
    num_images_viewed++;
    return;
}
...
#ifndef DISABLE_GUI
/* Exit the program with a dialog box message indicating that the user
   has viewed a large number of images. */
void too_many_images(GtkWindow *parent) {
    GtkDialogFlags flags = GTK_DIALOG_DESTROY_WITH_PARENT;
    const char *msg = "You've looked at a lot of images, "
        "you should take a break.";
    GtkWidget *md =
        gtk_message_dialog_new(parent, flags,
...
static void on_open_image(GtkButton* button, gpointer user_data) {
    ...
    if (num_images_viewed > 15)
        too_many_images(GTK_WINDOW(toplevel));
    ...
    {
        gint height = gtk_widget_get_allocated_height(GTK_WIDGET(button));
        if (height > 100) {
            /* This is more of an easter-egg than a vulnerability. */
            fprintf(stderr, "How did the button get so big?\n");
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    per_image_callback = &count_image;
